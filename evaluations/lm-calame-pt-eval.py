...
def main(args):
    ...
    dataset = load_dataset("NOVA-vision-language/calame-pt", "all", split="train")
    dataset = dataset.to_pandas()

    tokenizer = AutoTokenizer.from_pretrained(args.model_name, revision=args.revision, token=args.token)
    model = AutoModelForCausalLM.from_pretrained(args.model_name, revision=args.revision, token=args.token, attn_implementation="sdpa", torch_dtype=precision)
    ...
    output_file = f"{args.output_path}/calame_pt.json"
    with open(output_file, "w") as f:
        json.dump(results, f)

    print(f"Accuracy on CALAME-PT: {acc * 100:.2f}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate text generation model on CALAME-PT dataset")
