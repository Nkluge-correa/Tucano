...
def main(specs, slurm_job_id, hardware):
    ...
    # Get the world size from the environment variable.
    if "WORLD_SIZE" and "SLURM_PROCID" in os.environ:
        ...
        if world_size > 1:
            ...
            # Initialize the process group and set the device to the current rank.
    ...
    # Load the tokenizer from the HuggingFace Hub. 
    ...
    if args.resume_from_checkpoint:
        ...
        # Resize the token embeddings to the size of the instruct tokenizer (adds the <instruction> and </instruction> tokens)
        ...
        if master_process:
            ...
            with open(log_file, "a") as f:
                f.write(f"Initializing model from: {checkpoint_path}\n")
                f.write(f"Number of trainable parameters: {params:,}\n")
    
    else:

        raise ValueError("You need to provide a checkpoint to resume from. This is a fine-tuning script.")
    ...
    if master_process:
        ...
        if args.wandb_token is not None: 
            ...
            wandb.init(
                project="Tucano", 
                notes="Training a Llama model on a custom Portuguese-BR dataset.",
    ...
    for completed_steps in range(max_steps):
        ...
        if completed_steps % args.checkpointing_steps == 0 and completed_steps > 0:
            ...
            if master_process:
                ...
                if args.wandb_token is not None:
                    wandb.log({"stf_val_loss": val_loss_accum.item()})
        ...
        if master_process:
            ...
            if args.wandb_token is not None:
                ...
                wandb.log({
                            "stf_loss": accumulated_loss.item(),
                            "stf_lr": lr, 
                            })
    ...
    if master_process:
        ...
        output_dir = os.path.join(args.checkpoint_dir, "step_" + str(completed_steps + 1))
        os.makedirs(output_dir, exist_ok=True)
        checkpoint_path = os.path.join(output_dir, "checkpoint.pt")
        raw_model.save_pretrained(output_dir)
        tokenizer.save_pretrained(output_dir)
...
# python sft-tucano.py --specs "specs.yml" --slurm-job-id $SLURM_JOB_ID --hardware "a100"